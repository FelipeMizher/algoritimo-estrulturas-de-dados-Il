...
class Pokemon{
    ...
    public static int compararData(Log log, Pokemon data1, Pokemon data2){
        int resposta = 0;
        Date data1Date = data1.getCaptureDate();
        Date data2Date = data2.getCaptureDate();
    
        if(data1Date != null && data2Date != null)
            if(data1Date.before(data2Date)){ 
                resposta = -1;
                log.incrementaCount();
            } else if(data1Date.after(data2Date)){  
                resposta = 1;
                log.incrementaCount();
            } else{
    ...
    public static void insercao(Pokemon p[], Log log, int n){
        for(int i = 1; i < n; i++){
		    Pokemon tmp = p[i];
            int j = i - 1;
            while((j >= 0) && (compararData(log, tmp, p[j])) == -1){
                p[j + 1] = p[j];
    ...
    public static void main(String[] args) throws IOException{
        ...
        Pokemon.insercao(pokemonArray, log, pokemonArray.length);
