...
class Pokemon{
    ...
    public static int Maior(Pokemon array[], int tamanhoHeap){
    ...
    public static void countingSort(Pokemon p[], int tamanhoHeap, Log log){
        int[] count = new int[Maior(p, tamanhoHeap) + 1];
        Pokemon[] ordenado = new Pokemon[tamanhoHeap];
        for (int i = 0; i < count.length; count[i] = 0, i++);
  
        for (int i = 0; i < tamanhoHeap; count[p[i].getCaptureRate()]++, i++);
  
        for(int i = 1; i < count.length; count[i] += count[i-1], i++);
  
        for(int i = tamanhoHeap-1; i >= 0; ordenado[count[p[i].getCaptureRate()]-1] = p[i], count[p[i].getCaptureRate()]--, i--);
  
        for(int i = 0; i < tamanhoHeap; p[i] = ordenado[i], log.incrementaMovimentacoes(),i++);

        sort(p, tamanhoHeap, log);
    ...
    public static void sort(Pokemon array[], int tamanhoHeap, Log log){
        for(int i = 0; i < tamanhoHeap; i++){
            int menor = i;
            for(int j = i + 1; j < tamanhoHeap; j++){
                if(array[menor].getCaptureRate() == array[j].getCaptureRate() && array[menor].getName().compareTo(array[j].getName()) > 0){
                    menor = j;
                }
                log.incrementaCount();
            }
    ...
    public static void main(String[] args) throws IOException{
        ...
        Log log = new Log("/tmp/821811_countsort.txt");
        ...
        Pokemon.countingSort(pokemonArray, pokemonArray.length, log);
