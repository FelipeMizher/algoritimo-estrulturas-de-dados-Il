...
class Personagem{
    ...
    public static boolean comparar(Personagem p1, Personagem p2, Log log){
        boolean resposta = false;

        if(p1.getHairColour().compareTo(p2.getHairColour()) > 0){  
            resposta = true;
            log.incrementaCount();
        } else if(p1.getHairColour().compareTo(p2.getHairColour()) < 0){
            resposta = false;
            log.incrementaCount();
        } else{
    ...
    public static void heapsort(Personagem personagens[], Log log, int n){
        ...
        for(int tamanhoHeap = 2; tamanhoHeap <= n; tamanhoHeap++){
           construir(personagens,tamanhoHeap, log);
        }
        int tamanhoHeap = n;
        while(tamanhoHeap > 1){
           swap(personagens, 1, tamanhoHeap--, log);
           reconstruir(personagens, tamanhoHeap, log);
        }
        tmp = personagens;
        personagens = new Personagem[n];
        for(int i = 0; i < n; i++){
    ...
    public static int getMaiorFilho(Personagem array[], int i, int tamanhoHeap, Log log){
        int filho;
        if (2*i == tamanhoHeap || comparar(array[2*i], array[2*i+1], log)){
           filho = 2*i;
        } else {
           filho = 2*i + 1;
        }
    ...
    public static void construir(Personagem array[], int tamanhoHeap, Log log){
        for(int i = tamanhoHeap; i > 1 && comparar(array[i], array[i/2], log); i /= 2){
           swap(array, i, i/2, log);
        }
     }

     public static void reconstruir(Personagem array[], int tamanhoHeap, Log log){
        int i = 1, filho;
    ...
    public static void main(String[] args) throws Exception{
        ...
        log = new Log("/tmp/821811_heapsort.txt");
        tamanho = Pesquisar_personagem(personagens);
        start = Instant.now();
        heapsort(personagens, log, tamanho);
        end = Instant.now();
