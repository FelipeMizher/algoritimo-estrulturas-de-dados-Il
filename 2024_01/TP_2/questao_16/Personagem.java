...
class Personagem{
    ...
    public static void insercaoParcial(Personagem personagens[], Log log, int n){
        for(int i = 1; i < n; i++){
		    Personagem tmp = personagens[i];
            int j = i - 1;
    ...
    public static void main(String[] args) throws Exception{
        ...
        log = new Log("/tmp/821811_insercaoParcial.txt");
        tamanho = Pesquisar_personagem(personagens);
        start = Instant.now();
        insercaoParcial(personagens, log, tamanho);
        end = Instant.now();
