...
bool comparar(Personagem a, Personagem b){
    bool resposta = false;
    char aEye[100], bEye[100]; 

    strcpy(aEye, a.eyeColour);
    strcpy(bEye, b.eyeColour);

    if(strcmp(aEye, bEye) < 0){       
        resposta = true; 
        count++;  
    } else if(strcmp(aEye, bEye) > 0){  
        resposta = false;
        count++;
    } else{                
        count++;   
...
void inserir(int n, int cor, int h){

    for(int i = (h + cor); i < tamSubArray; i += h){
        Personagem tmp = sub[i];
        int j = i - h;

        while((j >= 0) && (!comparar(sub[j], tmp))){
            movimentacoes++;
            sub[j + h] = sub[j];
            j -= h;
        }
...
void registroLog(){
    ...
    arquivo = fopen("/tmp/821811_shellsort.txt", "w");
