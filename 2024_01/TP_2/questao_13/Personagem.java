...
class Personagem{
    ...
    public static boolean comparar(Personagem a, Personagem b, Log log){
        ...
        if(a.getActorName().compareTo(b.getActorName()) > 0){
            resposta = false;
            log.incrementaCount();
        } else if((a.getActorName().compareTo(b.getActorName()) < 0)){
            resposta = true;
            log.incrementaCount();
        } else{
    ...
    public static void intercalar(Personagem array[], int esq, int meio, int dir, Log log){
        int n1, n2, i, j, k;
        n1 = meio - esq + 1;
        n2 = dir - meio;
     ...
     public static void merge(Personagem sub[], int tam, Log log) {
        mergesort(sub, 0, tam-1, log);
        Exibir(sub);
     }

    public static void mergesort(Personagem array[], int esq, int dir, Log log){
        if (esq < dir){
            int meio = (esq + dir) / 2;
    ...
    public static void main(String[] args) throws Exception{
        ...
        log = new Log("/tmp/821811_mergesort.txt");
        tamanho = Pesquisar_personagem(personagens);
        start = Instant.now();
        merge(personagens, tamanho, log);
        end = Instant.now();
