...
class Personagem{
    ...
    public static int compararData(Log log, Personagem data1, Personagem data2){
        int resposta = 0;
        LocalDate data1Date = data1.getDateOfBirth();
        LocalDate data2Date = data2.getDateOfBirth();

        if(data1Date.isBefore(data2Date)){ 
            resposta = -1;
            log.incrementaCount();
         } else if(data1Date.isAfter(data2Date)){  
            resposta = 1;
            log.incrementaCount();
         } else{
    ...
    public static int Pesquisar_personagem(Personagem[] p) throws Exception{
        ...
        file = new FileReader("C:/Users/felip_000/Desktop/exercicos/TP_2/characters.csv/");
    ...
    public static void insercao(Personagem personagens[], Log log, int n){
        for(int i = 1; i < n; i++){
		    Personagem tmp = personagens[i];
            int j = i - 1;
            while((j >= 0) && (compararData(log, tmp, personagens[j])) == -1){
    ...
    public static void main(String[] args) throws Exception{
        ...
        log = new Log("/tmp/821811_insercao.txt");
        tamanho = Pesquisar_personagem(personagens);
        start = Instant.now();
        insercao(personagens, log, tamanho);
        end = Instant.now();
