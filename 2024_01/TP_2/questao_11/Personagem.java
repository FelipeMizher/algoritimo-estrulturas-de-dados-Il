...
class Personagem{
    ...
    public static int Maior(Personagem array[], int tamanhoHeap){
    ...
    public static void countingSort(Personagem personagens[], int tamanhoHeap, Log log){
        int[] count = new int[Maior(personagens, tamanhoHeap) + 1];
        Personagem[] ordenado = new Personagem[tamanhoHeap];
        for (int i = 0; i < count.length; count[i] = 0, i++);
  
        for (int i = 0; i < tamanhoHeap; count[personagens[i].getYearOfBirth()]++, i++);
  
        for(int i = 1; i < count.length; count[i] += count[i-1], i++);
  
        for(int i = tamanhoHeap-1; i >= 0; ordenado[count[personagens[i].getYearOfBirth()]-1] = personagens[i], count[personagens[i].getYearOfBirth()]--, i--);
  
        for(int i = 0; i < tamanhoHeap; personagens[i] = ordenado[i], log.incrementaMovimentacoes(),i++);

        sort(personagens, tamanhoHeap,log);
        Exibir(personagens);
    }

    public static void sort(Personagem array[], int tamanhoHeap, Log log){
        for(int i = 0; i < tamanhoHeap; i++){
            int menor = i;
    ...
    public static void main(String[] args) throws Exception{
        ...
        log = new Log("/tmp/821811_countingsort.txt");
        tamanho = Pesquisar_personagem(personagens);
        start = Instant.now();
        countingSort(personagens, tamanho, log);
        end = Instant.now();
