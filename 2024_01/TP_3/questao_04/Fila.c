...
int media(){
    int soma = 0, count = 0, media = 0;

    for(int i = primeiro; i != ultimo; i = ((i + 1) % (6))){
        count++;
        soma += personagens[i].yearOfBirth;
    }
    media = soma/count;
...
Personagem remover(){
    Personagem resposta = personagens[primeiro];
...
void inserir(Personagem novo){
    if(((ultimo + 1) % (6)) == primeiro){
        Personagem remove = remover();
    }

    personagens[ultimo] = novo;
    ultimo = (ultimo + 1) % (6);
